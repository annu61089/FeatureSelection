...
public class Datumbox {
    ...
    public static void main(String[] args) throws  Exception{
        ...
        if (args.length < 1)
        {
            System.out.println("Usage: java -jar <jar_path>  <dataset_location> [<FeatureCount>] [<aLevel>]" );
            return;
        }
        int featureCount = 20;
        double aLevel = 0.7;
        if (args.length > 1) {
            featureCount = Integer.valueOf(args[1]);
        }
        if (args.length > 2) {
            aLevel = Double.parseDouble(args[2]);
        }

        System.out.println("Reading data from " + args[0]);
        featureSelection.testSelectFeatures(args[0], featureCount, aLevel);
    }
